...
namespace KitchenCryopreservation.Patches
{
    [HarmonyPatch]
    static class RemoveToolsAtNight_Patch
    {
        static readonly Type TARGET_TYPE = typeof(RemoveToolsAtNight);
        const bool IS_ORIGINAL_LAMBDA_BODY = false;
        const int LAMBDA_BODY_INDEX = 0;
        const string TARGET_METHOD_NAME = "OnUpdate";
        const string DESCRIPTION = "Prevent destroy tools"; // Logging purpose of patch
